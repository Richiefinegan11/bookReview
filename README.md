...
Once the user has registered or logged in, they will be directed to their profile which will be created for them and will show a welcoming message. On their profile page, the user will be shown text with the number of books they have reviewed versus the books reviewed, the profile page will also have their review displayed. For users who have only registered, they can navigate easily to the "Write Review" tab which will bring them to a form, where they can write their review.
...
* As a user who enjoys giving their opinions on books they have read, they can create a profile, and review a book on the write review page
...
#### Login
The Login page is very similar to the Register page, there is a form at the top of the page, where the user is prompted to type in their username and password. The form will on except characters from 5 to 15 in length and numbers and letters, the user must match this or a prompt will instruct them to match the correct format.
...
Under these forms, there is an introductory question. This is followed by nice images of books and text explaining the aim of the site to users. On large devices, the text and image position change from left to right down the page, to give a more aesthetically pleasing feel to the page. On smaller and mobile devices, they adapt to what looks more pleasing on the mobile device. 

### Book Review 
At the top of the Book Review page, there is a responsive search bar, this allows the user to search for a book review, based on the book title, author, or genre. Under the search bar, there is a search button for the user to commence the search or a reset button, which will reset the page. Next on the page is the book reviews from all the users. This is laid out on cards with an image and text as card content. The text has a clickable card reveal "See Review" link, which reveals the book's review. Under this is the Title of the book, followed by the author and the genre. When a user uploads a review, the books will neatly and responsively fall into line. The last piece of text on the card is the link that will take the user to an external site to buy the book, the site will then gain commission for directing the user through their site.
...
### Profile
At the top of the profile page, once the user has logged in or registered, they are greeted by a welcome message, indicating to them the number of books they have reviewed versus the total amount reviewed. Under this message, if the user has written a review, it will display in a similar format to the book review page. The books displayed on this page will only be books the in-session user has reviewed. 
On each book reviewed by the user, there is an edit and a delete button. 
When the user clicks the edit button, it will bring the user to a separate page, which has a form. The form will be prefilled with the chosen book's info, for ease of editing. When the user is finished editing, they can then click the edit button, once this is done, they will be redirected to their profile and a message will flash "Your Review Has Been Updated". There is a cancel button on the edit review page which brings the user back to their profile.
...
* Blog - Once the site gets some more interest and traffic, I will add a blog page to the site's users can then interact with each other online too.
...
* [PYTHON3](https://www.python.org)
  * Python3 was used to help simplify writing the code and reduce the time needed for this project
* [FLASK](https://flask.palletsprojects.com/en/1.1.x/)
  * The project used the Flask framework for ease
...
## Database Used
MongoDB is a general-purpose, document-based, distributed database built for modern application developers and the cloud era. MongoDB is a NoSQL database program. I created the following collections for my project:

* books: This was used to store the user's reviews and was displayed on multiple pages such as the "book review" page and their profile.
* users: This was used to store the user's login details when they register for the site and then was grabbed for logging in. It also provided data for the profile page and other links to be used
